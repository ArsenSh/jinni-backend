// Jinni V3 chat endpoint — /chat-stream-v3, parallel to v2 (which stays
// untouched and default) exactly as v2 sits beside v1. See the V3 doc §12.
//
// ONE change from v2: the message's MEANING is decided by the conversation
// controller (engine/controller/conversationController.js) — one state-aware
// model call that replaces the regex fast path, the DeepSeek classifier and
// the keyword lane rules. Every lane's execution, every tool and every
// honesty guard below is v2's, unchanged. The controller's lane outranks the
// keyword flags (see the V3 block before `questionTurn`); when it fails the
// turn runs exactly as v2 would.
//
...
const conversationController = require('../engine/controller/conversationController');
...
router.post('/chat-stream-v3', auth, usageTracker, async (req, res) => {
    ...
    const recentTurns = withServerReply(recentTurnsFromMessages(sessionPeek?.messages, 10), sessionPeek?.lastReply);
    ...
    try {
        ...
        console.warn('[v3] taste load failed:', tasteErr.message);
    }
    ...
    try {
        if (req.userLimit) {
            ...
            if (usageStatus.onCooldown) {
                ...
                console.warn(`[v3][limits] 429 user=${req.user.id} until=${usageStatus.cooldownUntil} tokensUsed=${usageStatus.dailyTokensUsed} tokensRemaining=${usageStatus.dailyTokensRemaining}`);
        ...
        console.log(`[v3][limits] blocked: ${limitError.message}`);
    ...
    // written to ChatTurn — the same facts the [v3] log line prints, but as
    ...
    try {
        ...
        try {
            ...
            // V3: one state-aware controller call instead of fast path +
            // classifier + keyword rules. Falls back to v2's classifier.
            // Where the traveler IS: GPS through the gazetteer (no API call),
            // else the saved location. The origin of any flight they do not
            // state — without it the controller asked "from which city?".
            const _gps = (Number.isFinite(Number(location?.lat)) && Number.isFinite(Number(location?.lng)))
                ? { lat: Number(location.lat), lng: Number(location.lng) } : null;
            const _here = _gps ? await resolveRegion({ center: _gps }).catch(() => null) : null;
            const _saved = user?.settings?.location || null;
            const travelerLocation = _here && (_here.city || _here.country)
                ? [_here.city, _here.country].filter(Boolean).join(', ')
                : (_saved?.city ? [_saved.city, _saved.country].filter(Boolean).join(', ') : null);
            v3Decision = await conversationController.decide({
                message, recentTurns, userLanguage, appCfg,
                dateNote: describeDate(buildTimeContext({ timezone: userTimezone, lng: Number.isFinite(Number(location?.lng)) ? Number(location.lng) : null })),
                state: {
                    travelerLocation,
                    ...
                    lastDeck: (() => {
                        ...
                        for (let i = ms.length - 1; i >= 0; i--) {
                            const r = ms[i]?.recommendations;
                            if (Array.isArray(r) && r.length) return r.map(x => x?.name).filter(Boolean).slice(0, 8);
                        }
                        return [];
                    })(),
            ...
            intent = v3Decision.intent;
            console.log(`[v3] controller ${v3Decision.source} lane=${v3Decision.lane || '-'}${v3Decision.answersPendingQuestion ? ' answers-question' : ''}${v3Decision.topicChanged ? ' topic-changed' : ''}${v3Decision.flights ? ' flights=' + JSON.stringify(v3Decision.flights) : ''} model=${v3Decision.model} ${v3Decision.ms}ms${v3Decision.error ? ' error=' + v3Decision.error : ''}`);
            ...
            console.warn('[v3] intent failed, treating as place query:', err.message);
        ...
        let deckAsk = intent.isTravel && !intent.infoAsk
            && (countOrRefillAsk
            ...
            if (hit) {
                for (let i = (sessionPeek?.messages || []).length - 1; i >= 0; i--) {
                    ...
                    if (recs.length > 1) {
                        ...
                        if (recs[idx]) { namedCard = recs[idx]; console.log(`[v3] bridge ordinal: "${hit}" → "${namedCard.name}"`); }
                        break;
                    }
                }
            }
        ...
        if (!namedCard && intent.isTravel && !deckAsk && _referential && !_redoAsk && !namesVenueType(message)) {
            ...
            if (prev) {
                ...
                if (namedCard) console.log(`[v3] bridge follow-up: place carried from previous turn ("${namedCard.name}")`);
            }
        }
        ...
        if (!namedCard && parseReferentAsk(message)) {
            ...
            if (ld?.name && _placeLive(ld.name) && (!refAt || (ld.at && new Date(ld.at) > new Date(refAt)))) {
                ...
                console.log(`[v3] referent ask -> "${ld.name}" (last discussed place)`);
            } else if (ref && _placeLive(ref.name)) {
                namedCard = ref;
                console.log(`[v3] referent ask -> "${ref.name}" (last deck's top card)`);
            } else if (ld?.name || ref) {
                ...
                contextualQ = true;
                console.log('[v3] referent ask -> conversation context (last reply was not about a place)');
            } else {
                referentClarify = true;
                console.log('[v3] referent ask with nothing shown -> clarify, no deck');
            }
        }
        ...
        if (!namedCard && !deckAsk && message.trim().split(/\s+/).length <= 4) {
            ...
            if (cand) {
                ...
                if (isTransportAsk(prevU)) {
                    namedCard = cand;
                    forceTransport = true;
                    console.log(`[v3] bare-name follow-up: "${message.trim()}" → "${namedCard.name}"`);
                }
            }
        }
        let transportAsk = intent.infoAsk === 'transport'
        ...
        let infoTurn = !intent.isTravel || intent.infoAsk === 'how_to';
        ...
        if (!placeQuestion && !countOrRefillAsk && isEntityQuestion(message)) {
            placeQuestion = true;
            console.log('[v3] entity question -> tool loop (deterministic shape match)');
        }
        ...
        // not theirs to touch.
        // ── V3: the controller's lane outranks every keyword flag above ──
        if (v3Decision?.lane) {
            const f = resolveLaneFlags(v3Decision.lane, {
                transportAsk, placeQuestion, deckAsk, namedCard, referentClarify, contextualQ,
                isTravel: intent.isTravel, infoAsk: intent.infoAsk, actionType: intent.actionType, destinationScope: intent.destinationScope,
            });
            ({ transportAsk, placeQuestion, deckAsk, namedCard, referentClarify, contextualQ } = f);
            intent.isTravel = f.isTravel; intent.infoAsk = f.infoAsk; intent.actionType = f.actionType; intent.destinationScope = f.destinationScope;
            infoTurn = !intent.isTravel || intent.infoAsk === 'how_to';
        }
        ...
        const resolveAskedRegion = async () => {
            ...
            try {
                ...
                console.warn('[v3] asked-region resolve failed, using ambient:', err.message);
            }
            return ambient;
        };
        ...
        try {
            ...
            if (dest.source !== 'gps') {
                console.log(`[v3] centre=${dest.source}${dest.city ? ` "${dest.city}"` : ''} (${center?.lat?.toFixed(3)},${center?.lng?.toFixed(3)})`);
            }
            ...
            if (dest.remember && sessionId) {
                require('../models/ChatSession')
                    ...
                    .catch(err => console.warn('[v3] activeDestination save failed:', err.message));
            }
            }
        } catch (err) {
            console.warn('[v3] destination resolve failed, keeping GPS centre:', err.message);
        }
        ...
        if (refillActive) {
            ...
            meta.refill = true;
            console.log(`[v3] refill → continuing the ask that made the deck: "${String(prevUserAsk).slice(0, 60)}"`);
        }
        ...
        if (v3Decision?.lane === 'clarify' && v3Decision.clarifyQuestion) {
            ...
            console.log(`[v3] clarify → "${reply.slice(0, 80)}"`);
        } else if (mapAsk && sessionCards.length) {
            ...
            stats.path = 'map';
            console.log(`[v3] map-ask re-served ${recommendations.length} card(s)${target ? ` for "${target.name}"` : ' (newest deck)'}`);
            ...
            const gaMessages = buildGettingAroundMessages({
                ...
                // The controller's resolved ask (cities, windows, stay) — the
                // narrator calls find_flights with exactly this.
                flightPlan: v3Decision?.flights || null,
                localFacts: gaFacts,
            ...
            if (namedCard) {
                ...
                console.log(`[v3] transport answer routes to shown card "${namedCard.name}"${recommendations.length ? ' (own map attached)' : ''}`);
            }
            ...
            console.log(`[v3] settings: ${done.length ? done.join('; ') : 'nothing applied'}`
            ...
            stats.path = 'chitchat';
            console.log('[v3] itinerary-shaped ask -> clarifier hand-off (days=' + (prefillDays == null ? '?' : prefillDays) + ', hotel=' + (_det.hotel ? 'stated' : '?') + ', breakfast=' + (_det.breakfast == null ? '?' : _det.breakfast) + (_itinDest ? `, dest=${_itinDest.name}` : '') + ') — same build path as the quick action');
            }
            ...
            stats.path = 'chitchat';
            console.log(`[v3] destination-scale ask -> no deck${_excluded.length ? ` · ruled out: ${_excluded.join(', ')}` : ''}`
            ...
            stats.path = 'currency';
            console.log(`[v3] currency: ${_fx.amount} ${_fx.from} -> ${fxOut != null ? _fmt(fxOut) : 'no rate'} ${_fx.to} (deterministic, $0)`);
            ...
            if (_doc && loop.text) {
                ...
                if (!already) {
                    ...
                    console.log(`[v3] first-mention card attached: "${_doc.name}"`);
                }
            }
            ...
            stats.path = 'chitchat';
            addUsage(out);
            console.log(`[v3] ${intent.infoAsk ? `info(${intent.infoAsk})` : 'chit-chat'} narrated in ${Date.now() - t0}ms (${out.usage.in}/${out.usage.out} tok)${infoFacts.length ? ` facts=${infoFacts.map(f => f.sourceName).join('+')}` : ''}`);
        } else if (!center) {
            ...
            if (!sizedByPopulation && (meta.destScale || 'town') === 'town' && radiusKm > 15 && (
                ...
                if (meta.centreSource === 'stated') {
                    console.log(`[v3] stated position -> ${radiusKm}km (a spot you stand on is not a 50km sweep)`);
                }
            }
            ...
            if (alsoTypes) console.log(`[v3] broad ask -> also ${alsoTypes.slice(1).join(', ')}`);
            ...
            if (askedRadiusKm) {
                ...
                console.log(`[v3] explicit radius: ${askedRadiusKm}km`);
            }
                ...
                try {
                    ...
                    if (ends[0] && ends[1]) {
                        ...
                        if (corridorCentresList.length) {
                            ...
                            console.log(`[v3] corridor ${ends[0].name} → ${ends[1].name}: ${corridorCentresList.length} segment(s)`);
                        }
                    }
                } catch (err) {
                    console.warn(`[v3] corridor failed: ${err.message} — answering from the named centre`);
                }
            }
            ...
            if (!result.places.length && ['all_filtered', 'no_candidates'].includes(result.reason)
                ...
                if (townNarrowing || subsetTokens.length) {
                    ...
                    if (rerun.places.length) {
                        result = rerun;
                        meta.reServed = true;
                        console.log(`[v3] narrowing ask re-serves ${rerun.places.length} shown place(s) (${townNarrowing ? 'named town' : `matched: ${subsetTokens.join(',')}`})`);
                    }
                }
            }
            ...
            if (result.degraded || !result.places.length) {
                ...
                if (sessionCards.length && /[?？՞]\s*$/.test(String(message).trim())) {
                    try {
                        ...
                        console.warn(`[v3] empty-deck tool rescue failed: ${err.message} — falling back to the empty reply`);
                    }
                }
                if (!rescued) {
                    ...
                    try {
                        ...
                        console.warn(`[v3] empty-deck narrator failed (${err.message}) — English fallback`);
                        if (!streamedAny) { reply = fallback; send(res, { type: 'token', content: reply }); }
                    }
                    console.log(`[v3] empty deck: cause=${cause}${result.provenance.openNowDropped ? ` openNowDropped=${result.provenance.openNowDropped}` : ''} city=${emptyCity || 'n/a'} lang=${langName}`);
                }
            } else if (
                ...
                stats.path = 'no_match';
                console.log(`[v3] relevance brake: nothing matches [${result.provenance.unmatched.join(',')}] — answered without cards (${Date.now() - t0}ms)`);
            } else if (
                ...
                console.log(`[v3] junk-question brake: lex=0 question/correction "${String(message).slice(0, 40)}" → tool loop (${loop.toolCalls.length} call(s)), no deck`);
            } else {
                ...
                // and geo tokens alone never count (messageNamesPlace).
                {
                    ...
                    if (namedHits.length && namedHits.length < result.places.length) {
                        console.log(`[v3] name-ask: serving ${namedHits.length}/${result.places.length} — the message names them`);
                    ...
                    for (const p of namedHits) {
                        ...
                        }]).catch(err => console.warn(`[v3] name-ask flag failed for ${p.placeId}: ${err.message}`));
                        if (quarantine) console.log(`[v3] name-ask quarantine: "${p.name}" pending staff verdict`);
                    }
                }
                ...
                if (category && result.places?.length) {
                    ...
                    if (shownIds.length) {
                        ...
                        ).catch(err => console.warn(`[v3] action tagging failed: ${err.message}`));
                    }
                }
                const promptArgs = {
                ...
                try {
                    ...
                    console.warn(`[v3] streamed narration failed (${err.message}) — one-shot fallback`);
                }
                if (!streamedOk) {
        ...
        console.error('[v3] turn failed:', err.message);
        stats.path = 'error';
    ...
    try {
        if (req.userLimit) {
            ...
            console.log(`[v3][limits] tok est=${estimatedTokens} actual=${actualTokens || 'n/a'} charged=${estimatedTokens + delta} places+${uniquePlaces.size}`);
        }
    } catch (e) {
        console.warn(`[v3][limits] post-stream usage true-up skipped: ${e.message}`);
    }
    ...
    try {
        require('../models/ChatTurn').record({
            ...
            controllerLane: v3Decision?.lane || null,
            controllerSource: v3Decision?.source || null,
            log: require('../engine/utils/requestLog').capture(),
        });
    } catch (e) { console.warn('[v3][turnlog] skipped:', e.message); }
